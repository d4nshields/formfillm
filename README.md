...
Ollama is the default and needs no setup. Because formfillm is a thin client of the OpenAI-compatible `POST /v1/chat/completions`, advanced users can instead run any local server that speaks it and switch with **Settings → Backend** (a dropdown that fills in the right default URL and thinking-off behavior). No rebuild — the manifest already permits any loopback port. These engines also *grammar-enforce* the JSON schema (stronger than Ollama) and matter most for a shared many-user GPU box. Per-backend setup lives in **[docs/backends/](./docs/backends/)** (one page each for [Ollama](./docs/backends/ollama.md), [SGLang](./docs/backends/sglang.md), [vLLM](./docs/backends/vllm.md), [llama.cpp](./docs/backends/llamacpp.md)); the *why* (concurrency, hardware path) is in [docs/SCALING-LLM.md](./docs/SCALING-LLM.md).
